...
const materials = () => {
  ...
  const request = (reset_offset = true) => {
    ...
    axios
      .post(
        new URL("/api/v1/supplier/materials/list", themeConfig.backendUrl),
        query
      )
      .then((res) => {
        if (res.data.error) {
    ...
    MySwal.fire({
      ...
      if (result.value) {
        axios
          .delete(
            new URL(
              `/api/v1/supplier/materials/delete/${row.id}`,
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        setMaterialgroup(response.data.data.rows);
      })
      .catch((error) => {
  ...
  const AddModal = () => {
    ...
    const onSubmit = (e) => {
      ...
      axios
        .post(
          new URL("/api/v1/supplier/materials/create", themeConfig.backendUrl),
          {
            ...form,
            material_group: selectedOption.value.toString(),
                ...
                <div className="col-md-6 mt-1">
                  <div className="form-group">
                    ...
                    <Select
                      ...
                      options={materialgroup.map((option) => {
                        return {
                          label: option.name,
                          value: option.code,
                        };
  ...
  const EditModal = () => {
    ...
    const [form, setForm] = useState({
      ...
      material_group: editData.material_group,
    ...
    const onSubmitEdit = (e) => {
      ...
      axios
        .put(
          new URL("/api/v1/supplier/materials/update", themeConfig.backendUrl),
          form
        )
        .then((res) => {
          if (res.data.error) {
  ...
  const basicColumns = [
    ...
    },
    {
      ...
      selector: (row) => row.material_group_name,
    },
    {
      name: "Status",
      maxWidth: "150px",
