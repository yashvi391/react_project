...
const Tds = () => {
  ...
  const request = (reset_offset = true) => {
    ...
    axios
      .post(new URL("/api/v1/admin/tds/list", themeConfig.backendUrl), query)
      .then((res) => {
        if (res.data.error) {
    ...
    MySwal.fire({
      ...
      if (result.value) {
        axios
          .delete(
            new URL(
              `/api/v1/admin/tds/delete/${row.id}`,
  ...
  const AddModal = () => {
    ...
    const onSubmit = (e) => {
      e.preventDefault();
      axios
        .post(new URL("/api/v1/admin/tds/create", themeConfig.backendUrl), form)
        .then((res) => {
          if (res.data.error) {
  ...
  const EditModal = () => {
    ...
    const onSubmitEdit = (e) => {
      e.preventDefault();
      axios
        .put(new URL("/api/v1/admin/tds/update", themeConfig.backendUrl), form)
        .then((res) => {
          if (res.data.error) {
            toast.error(res.data.message);
          }
