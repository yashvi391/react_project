...
const Companies = () => {
  ...
  const request = (reset_offset = true) => {
    ...
    axios
      .post(
        new URL("/api/v1/admin/company/list", themeConfig.backendUrl),
        query
      )
      .then((res) => {
        if (res.data.error) {
    ...
    MySwal.fire({
      ...
      if (result.value) {
        axios
          .delete(
            new URL(
              `/api/v1/admin/company/delete/${row.id}`,
  ...
  const AddModal = () => {
    ...
    const onSubmit = (e) => {
      ...
      axios
        .post(
          new URL("/api/v1/admin/company/create", themeConfig.backendUrl),
          form
        )
        .then((res) => {
          if (res.data.error) {
  ...
  const EditModal = () => {
    ...
    const onSubmitEdit = (e) => {
      ...
      axios
        .post(
          new URL("/api/v1/admin/company/update", themeConfig.backendUrl),
          form
        )
        .then((res) => {
          if (res.data.error) {
  ...
  const basicColumns = [
    ...
    //     column: "departments",
    //     sortable: true,
    //     selector: row => (<><Link to={`/configuration/departments/groups/${row.id}`}>Link</Link></>)
